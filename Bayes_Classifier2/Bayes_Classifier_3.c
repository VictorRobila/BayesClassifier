...
int main(){
    ...
    for(float myPERT=PERT; myPERT >=0; myPERT-=0.05){
        ...
        for (int round=1; round <= trials; round++){
            ...
            trueClassArrayPerc(true_class,percentages, pc, ddim,cdim);
            ...
            fillClassArray(classarray, true_class, probabilities, cdim, ddim, 1-myPERT ,seed);

            // Bayes rule is used to assign a class to each measurement
            Bayes_Rule(classarray,cifd, assign, pc, cdim, ddim);
